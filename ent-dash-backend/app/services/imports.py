...
async def get_all_imports(db: AsyncSession, user_id: int) -> List[FileImport]:
    # For now, allow seeing all imports so admins can monitor progress across users
    # In a stricter environment, we would check for 'view-all-imports' permission
    stmt = select(FileImport).order_by(FileImport.created_at.desc())
